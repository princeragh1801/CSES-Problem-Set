...
void _solve(){
    ...
    cout<<0<<nl;
    for(int k = 2; k <= n; k++){
        // total number of places in chessborad k*k
        ll num = k*k;

        // total number of ways we can place knights on chessboard
        ll totalNumberofWays = (num*(num-1))/2;
        ll numberOfWaysKnightsAttack = 4*(k-1)*(k-2);
        ll numberOfWays = totalNumberofWays-numberOfWaysKnightsAttack;
